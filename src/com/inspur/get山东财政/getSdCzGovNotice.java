...
public class getSdCzGovNotice implements PageProcessor {
    ...
    public void process(Page page) {
        ...
        if (page.getUrl().regex("http:\\/\\/www\\.sdcz\\.gov\\.cn\\/sdczww\\.s\\?method=lanmu&cid=[\\d]*").match()) {
            String sRegular = "//table[@class='list_info_table']/tbody/tr/td/a/@href";
            // 获取列表页面的链接列表
        ...
        // 2. 如果是用户详细页面
            ...
            // 获取详情页面信息
            String title = page.getHtml().xpath("//div[@class='level3_title']/text()").get();
            String cTime = page.getHtml().xpath("//div[@class='level3_hits']/text()").get();
            // String cSourceCom = page.getHtml().xpath("//span[@id='source']/text()").get();
            String sInfo = page.getHtml().xpath("//div[@class='level3_content']/html()").get();
...
/*            String tableName = "newsinfo";
    ...
    public static void main(String[] args) {
        ...
        Spider.create(new getSdCzGovNotice()).addUrl(inUrl).thread(5).run();
